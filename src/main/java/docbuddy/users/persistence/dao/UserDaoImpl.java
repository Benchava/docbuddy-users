...
public class UserDaoImpl implements UserDao {
    ...
    private User entityToUser(Entity entity) {
        return User.builder().id(entity.getKey().getId()).admin(entity.getBoolean(User.Constants.ADMIN))
                .doctor(entity.getBoolean(User.Constants.DOCTOR)).firstName(entity.getString(User.Constants.FIRST_NAME))
                .lastName(entity.getString(User.Constants.LAST_NAME))
                .userName(entity.getString(User.Constants.USERNAME)).password(entity.getString(User.Constants.PASSWORD))
                .build();
    }
    ...
    public Long createUser(User user) {
        ...
        FullEntity<IncompleteKey> incUserEntity = Entity.newBuilder(key).set(User.Constants.ADMIN, user.isAdmin())
                .set(User.Constants.DOCTOR, user.isDoctor()).set(User.Constants.FIRST_NAME, user.getFirstName())
                .set(User.Constants.LAST_NAME, user.getLastName()).set(User.Constants.USERNAME, user.getUserName())
                .set(User.Constants.PASSWORD, BCryptUtil.encodePassword(user.getPassword()).getPassword()).build();
    ...
    public void updateUser(User user) {
        ...
        Entity entity = Entity.newBuilder(key).set(User.Constants.ADMIN, user.isAdmin())
                .set(User.Constants.DOCTOR, user.isDoctor()).set(User.Constants.FIRST_NAME, user.getUserName())
                .set(User.Constants.LAST_NAME, user.getLastName()).set(User.Constants.USERNAME, user.getUserName())
                .set(User.Constants.PASSWORD, user.getPassword()).build();
    ...
    public Result<User> getAllUsers(String startCursorString) {
        ...
        if (!Strings.isNullOrEmpty(startCursorString)) {
            startCursor = Cursor.fromUrlSafe(startCursorString);
        }

        Query<Entity> query = Query.newEntityQueryBuilder().setKind("User").setLimit(10).setStartCursor(startCursor)
                .build();
        ...
        if (Strings.isNullOrEmpty(user.getUserName())) {
            ...
            if (Strings.isNullOrEmpty(user.getPassword())) {
                throw new BadRequestException(
                        String.format("Bad request. Password for user is %s.", user.getPassword()));
            }
        }
    }
}
